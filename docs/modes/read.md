...
4. If there are no assistant messages:
   ...
   - Timeout completes the mode; the state machine enters `DeepSleep` when unplugged or `CheckAssistantMessage` when plugged.
...
6. While reading:
   ...
   - Idle timeout completes the mode; the state machine enters `DeepSleep` when unplugged or `CheckAssistantMessage` when plugged.
...
This keeps `Read` interruption cheap: a long press can abort reading and then enter `Recording`, whose own entry flow clears assistant messages through `clear_assistant_message`.
