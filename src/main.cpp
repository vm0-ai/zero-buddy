...
constexpr size_t kMaxRecordBytes = 16000 * 2 * 3;  // 3 seconds, 16kHz mono 16-bit
...
uint8_t* g_record_buffer = nullptr;
...
std::vector<String> g_pending_assistant_messages;
...
bool ensureRecordBuffer(String* error_out) {
  if (g_record_buffer != nullptr) {
    return true;
  }
  g_record_buffer = static_cast<uint8_t*>(malloc(kMaxRecordBytes));
  if (g_record_buffer == nullptr) {
    *error_out = "record alloc failed";
    return false;
  }
  return true;
}
...
String currentPendingAssistantMessage() {
  if (g_pending_message_index >= g_pending_assistant_messages.size()) {
    return "";
  }
  return g_pending_assistant_messages[g_pending_message_index];
}
...
void advancePendingAssistantMessage() {
  ...
  if (g_pending_message_index >= g_pending_assistant_messages.size()) {
    g_pending_assistant_messages.clear();
...
String tlsReadAll(mbedtls_ssl_context* ssl, String* error_out) {
  String out;
  uint8_t buf[512];
  while (true) {
    ...
    if (isTimeoutTlsError(ret) && out.length() > 0) {
      return out;
    }
...
bool readHttpStatusAndBody(const String& raw, int* status_out, String* body_out) {
  const int status_end = raw.indexOf("\r\n");
  if (status_end < 0) {
    return false;
  }
  const String status_line = raw.substring(0, status_end);
  ...
  *status_out = code_str.toInt();
  ...
  const int body_start = raw.indexOf("\r\n\r\n");
  *body_out = body_start >= 0 ? raw.substring(body_start + 4) : "";
  return true;
}
...
bool zeroProxyRequest(const String& method,
  ...
  zeroStage("read resp");
  String read_error;
  const String raw_response = tlsReadAll(&target_tls->ssl, &read_error);
  if (!readHttpStatusAndBody(raw_response, code_out, response_out)) {
    *error_out = "bad zero resp";
    Serial.println("ZERO raw response undecoded:");
...
bool pollZeroAssistantMessages(bool baseline_only, String* error_out) {
  ...
  int code = 0;
  String response;
  if (!zeroProxyRequest("GET", path, "", &code, &response, error_out)) {
    return false;
  }
  if (code != 200) {
  ...
  std::vector<String> assistant_messages;
  parseZeroMessagesResponse(response, &newest_message_id, &assistant_messages);
  if (!newest_message_id.isEmpty()) {
    g_zero_last_seen_message_id = newest_message_id;
  }
  if (!baseline_only) {
    for (const auto& msg : assistant_messages) {
      g_pending_assistant_messages.push_back(msg);
    }
    if (!assistant_messages.empty() && g_pending_assistant_messages.size() == assistant_messages.size()) {
      g_pending_message_index = 0;
...
bool pumpBufferedAudioToAsr(bool finalize, String* error_out) {
  ...
  while (g_record_sent_bytes < g_recorded_bytes) {
    const size_t remaining = g_recorded_bytes - g_record_sent_bytes;
    const size_t writable = min(kStreamChunkBytes - g_stream_chunk_fill, remaining);
    memcpy(g_stream_chunk + g_stream_chunk_fill, g_record_buffer + g_record_sent_bytes, writable);
    g_stream_chunk_fill += writable;
    g_record_sent_bytes += writable;

    if (g_stream_chunk_fill == kStreamChunkBytes) {
      if (!sendStreamAudioChunk(g_stream_chunk, g_stream_chunk_fill, false, error_out)) {
        return false;
      }
      g_stream_chunk_fill = 0;
  ...
  if (finalize) {
    if (g_stream_chunk_fill > 0) {
      if (!sendStreamAudioChunk(g_stream_chunk, g_stream_chunk_fill, true, error_out)) {
        return false;
      }
      g_stream_chunk_fill = 0;
    } else {
      if (!sendStreamAudioChunk(nullptr, 0, true, error_out)) {
        return false;
      }
    }
  }
...
void analyzeRecordedAudio() {
  analyzePcmBuffer(g_record_buffer,
                   g_recorded_bytes,
                   &g_record_peak,
                   &g_record_avg_abs,
                   &g_record_clipped_samples);
  Serial.printf("REC audio bytes=%u peak=%u avg=%u clipped=%u\n",
...
void loop() {
  ...
  if (millis() - last_heartbeat > 5000) {
    ...
    Serial.printf("alive wifi=%d ip=%s status=%s rec=%u\n",
  ...
  if (g_recording && bytes_read > 0) {
    ...
    const size_t writable = min(bytes_read, kMaxRecordBytes - g_recorded_bytes);
    if (writable > 0) {
      memcpy(g_record_buffer + g_recorded_bytes, g_audio_buffer, writable);
      g_recorded_bytes += writable;
    }
    const bool mode_uses_asr =
        (g_test_mode == TestMode::Normal || g_test_mode == TestMode::Asr);
    if (mode_uses_asr && !g_asr_session.active && !g_asr_connect_started &&
    ...
    if (mode_uses_asr && g_recording && g_asr_session.active) {
      String send_error;
  ...
  if (g_recording && g_test_mode != TestMode::Normal &&
